...
i=1;
 for i =[1 4:length('D:\groupFlow_trialDat\')] % first 4 pairs need to be checked by hand
    ...
    subjFolder = ([foldername '/' data(1:2)]);
    mkdir(subjFolder);
...
for epoch = 1:3:length(EEG.event)
    EEG = eeg_checkset( EEG ); % check the current data-set
    epoch_trial = [(EEG.event(epoch+2).latency-EEG.event(epoch).latency)/512+1]; % get the time of the trial
    
    EEG = pop_rmdat( EEG, EEG.event(epoch).type, [-1 epoch_trial+1] );
    
    % filename
    tNum = tNum +1;
    filename_trial = [subjfolder EEG.filename '_t' num2str(tNum)];
